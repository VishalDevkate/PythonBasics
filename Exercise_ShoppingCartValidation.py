...
def add_to_cart(items_to_add):
    global total_items_in_cart
    if items_to_add < 0:
        raise Exception("Cannot add a negative number of items.")
    total_items_in_cart = total_items_in_cart + items_to_add
    if total_items_in_cart > 5:
        raise Exception("Cart limit exceeded.")
    print(items_to_add,"items added. Total in cart: ",total_items_in_cart)

try:
    add_to_cart(2)  # Add 2 items
    #add_to_cart(-1)  # This should raise an exception
    add_to_cart(4)    # This should raise an exception
except Exception as e:
    print(e)